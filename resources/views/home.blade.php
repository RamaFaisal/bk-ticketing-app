<x-app-layout>
    ...
    <section class="max-w-7xl mx-auto py-12 px-6">
        <div class="flex justify-between items-center mb-8 md:flex-row flex-col">
            <h2 class="text-2xl font-black uppercase italic">Event</h2>
            <div class="flex gap-2 mt-4 lg:my-0">
                <a href="{{ route('home') }}">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            @foreach($events as $event)
                <x-event-card :title="$event->judul" :date="$event->tanggal_waktu" :location="$event->lokasi"
                    :price="$event->tickets->min('harga')" :image="$event->gambar" :href="route('events.show', $event)" />
            @endforeach
        </div>
    </section>
</x-app-layout>
